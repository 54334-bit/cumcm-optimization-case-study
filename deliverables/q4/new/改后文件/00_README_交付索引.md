...
| `01_提交件/result4-3.xlsx` | 波动电价下问题三的完整购电策略（4 表） | ✅ **已完成**（交付说明已回填；独立校验 9/9、`errors=[]`，见 `04_证据/q4_validator_result4-3.json`） |
...
| `06_图件/` | ✅ **已落地（2026-09-13，对话6 绘图线）**：6 张图 ×（SVG + 300dpi PNG）+ 图件 manifest + `README_图件索引.md`；源脚本 `6对话\code\q4_fig_main.py`，数据层 `6对话\code\q4_data_frozen.py`（锚点闸门 ANCHORS: PASS） |

## 四、待补（不影响两份提交件成立）

1. ~~`02_交付说明与附录/交付说明_Q4-3.md` —— 待回填~~ → ✅ **已完成**（含 Q3 v2 哈希 `39F4C8DF…`、读法 C 声明、全部关键数字）；
2. `03_论文素材/` —— ◐ **大部分完成**：2×2 反事实表（`三方共验_Q4交付件_裁定.md` §二）、机制段骨架、17.9% 参数推导、稳健性与裕度结论 ✅；**分块拆解表尚无论文成品**（现仅有 `04_证据/q4_price_block_decomp.json` 与图件 `06_图件/q4_06_effect_split`）；
3. ~~`06_图件/` —— 5 张图（SVG+300dpi PNG+manifest）~~ → ✅ **已完成**：实际交付 **6 张**（含图 5 反事实 2×2、图 6 分块分解），见 `06_图件/README_图件索引.md`；
4. ~~Q4-3 的独立复算报告~~ → ✅ **已完成**（`04_证据/q4_site_guard_report.md`，现场监理不复用我方脚本的独立复算）。

## 五、清单

> 哈希**唯一权威源**是同目录 `manifest.sha256.json`（现 71 条，含 `06_图件/`）。
> 下表两行属"自指记录"（`00_README_交付索引.md` 自身与 `manifest.sha256.json` 自身），
> 因文件改写会自我失效，**以 manifest 内的实际值为准**；其余各行与 manifest 逐条一致。

| 文件 | 字节 | sha256 |
| --- | ---: | --- |
| `00_README_交付索引.md` | —（**自指行**，值见 `manifest.sha256.json`） | —（自指行，以 manifest 为准） |
...
| `manifest.sha256.json` | —（**自指行**，值见文件本身） | —（自指行，以文件本身为准） |
