...
def main():
    ...
    ap.add_argument("--regime", choices=["q2", "q3"], default="q2",
                    help="费用列口径：q2=表1'全天购电费'仅计划费；q3=当日总费用（净退计划费+调整+当日紧急）")
    ...
    if a.regime == "q2":
        chk("S2b 表1 全天购电费 = Σ p[d,t]·G_t（位置映射 col=2+t，价格=%s）" % a.price, bad_fee == 0,
            {"bad_days": bad_fee, "n": n, "price": a.price, "tmpl": tmpl,
             "note": "若映射错位，此式必然失配 ⇒ 该检验同时验证位置映射；价格必须与文件的价格区制一致"})
    else:
        chk("S2b（q3 口径不适用，改由 S6b 判）", True,
            {"note": "q3 侧'全天购电费'= 当日总费用，不等于 Σp·G；位置映射由 S6a 的 J_plan/J_adj 精确一致间接验证"})
    ...
    if a.regime == "q3" and has_adj:
        ...
        chk("S6b 表1'全天购电费'= 当日总费用（**报告项**：受日均价近似限制）", True,
            {"bad_days": bad_day, "n_days": 334, "max_abs_diff": maxd,
             "note": "q3 侧该列 = 净退计划费 + 调整费 + 当日紧急费；因表3 只给段合计、本脚本用当日均价近似，"
                     "故本项只报告不做硬判定 —— **逐日精确复核由 q4_site_guard 用逐段真值完成**"})
