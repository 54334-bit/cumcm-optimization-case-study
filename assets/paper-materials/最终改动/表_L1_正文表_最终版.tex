...
\end{tabular}
\vspace{2pt}
\begin{flushleft}\footnotesize 注：问题 2 的购电量即计划购电量 G^{0}（非紧急部分按计划购电量全量计费）；「全天购电费（当日合计）」= \Sigma p\cdot G^{0}+\Sigma 5p H（H 为紧急购电量，明细见表 5），其下两行为「其中：计划购电费 / 紧急购电费」。提交件表内该列只含 \Sigma p\cdot G^{0}（计划费，不含紧急），引用时勿混。\end{flushleft}
\end{table}
\begin{table}[htbp]
\centering
...
\midrule
全天购电费（当日合计） & 41,595.10 & 19,334.88 & 44,095.97 & 61,446.07 \\
其中：计划购电费 & 40,498.51 & 19,334.88 & 43,914.08 & 61,446.07 \\
其中：紧急购电费 & 1,096.59 & 0.00 & 181.89 & 0.00 \\
\bottomrule
\end{tabular}
\vspace{2pt}
...
\end{tabular}
\vspace{2pt}
\begin{flushleft}\footnotesize 注：问题 3 的结算由计划购电量 G^{0} 与调整购电量 A 共同决定，故表 1 格式的「购电量」在此并列给出（（a）计划量、（b）调整量（最终执行量））；「全天购电费（当日合计）」= p\cdot\min(G^{0},A)+0.5p(G^{0}-A)^{+}+1.5p(A-G^{0})^{+}+5pH，其下四行为各分项。提交件表内该列为「该表购电量 × 电价」毛额（计划量/调整量毛额），与本表口径不同，引用时勿混。\end{flushleft}
\end{table}
\begin{table}[htbp]
\centering
...
项目 & 3.20 & 6.21 & 9.23 & 12.21 \\
\midrule
全天购电费（当日合计） & 43,711.19 & 18,955.18 & 44,537.63 & 62,175.13 \\
其中：计划净退费 & 39,804.28 & 18,784.09 & 42,388.71 & 60,028.12 \\
其中：违约退款 & 1,692.14 & 142.43 & 1,887.59 & 961.02 \\
其中：超购加价 & 374.95 & 15.61 & 17.08 & 625.42 \\
其中：紧急购电费 & 1,839.82 & 13.05 & 244.25 & 560.57 \\
\bottomrule
\end{tabular}
\vspace{2pt}
\begin{flushleft}\footnotesize 注：各费项为表内显示值之和，相加即当日合计。\end{flushleft}
