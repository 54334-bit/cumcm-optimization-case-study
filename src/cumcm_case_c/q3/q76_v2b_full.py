...
判据（跑前写死在文件头常量里）
------------------------------
* **Y1 收益**：全年 ``J_cash`` < 旧版 ``13,369,682.337449``（读法 C 全年现金，来自台账；**不是**表内第 147 列）。
...
147 列口径 = **本表购电量 × 该时段电价**（方案 A，2026-09-13 统一，4 位小数），可由逐段真值复现；
全年现金 ``J_cash`` 由台账给出、不落在表内。模板 sha 未变。
...
OLD_J_CASH = 13369682.337448763               # 旧版（v1）读法 C 全年现金（元）；该值不在交付表任何一列内
...
def cash_split(G0, A, H, price) -> dict:
    """读法 C 的逐段现金分解（台账口径；**不等于**交付表第 147 列，后者按方案 A 为本表购电量 × 电价）。
...
def old_cash(price) -> dict:
    """旧版逐段真值 × 附件 1 电价 ⇒ 读法 C 现金分解（用于台账对拍；第 147 列另按方案 A 核对）。"""
    with open(OLD_SEG, "r", encoding="utf-8") as fh:
        recs = [json.loads(line) for line in fh if line.strip()]
...
def y4_checks(xlsx: str, tpl: str, seg_records, price) -> dict:
    """Y4 物化四检：结构 / 表头 / 4h 分块 / 147 列合计（= 本表购电量 × 电价，方案 A；模板 sha 另计）。
    ...
    # ---- 147 列全年合计 vs 本表购电量 × 电价（方案 A，2026-09-13）----
    cash = {"J_plan": 0.0, "J_adj": 0.0, "J_emg": 0.0, "J_cash": 0.0}
    ...
    for s in main_sheets:
        key = "G0" if "计划" in s else "A"
        ref = float(sum(float((np.asarray(price, dtype=float) * np.asarray(day[key], dtype=float)).sum())
                        for day in seg_records))
        tot = 0.0
        for r in wb[s].iter_rows(min_row=2, values_only=True):
            if r[146] is not None:
                tot += float(r[146])
        rel = abs(tot - ref) / abs(ref) if ref else 0.0
        col.append({"sheet": s, "sum_col147": tot, "truth_sum_pq": ref,
                    "abs_diff": abs(tot - ref), "rel_diff": rel,
