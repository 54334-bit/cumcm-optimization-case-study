...
# 方案 A（2026-09-13 统一）：第 147 列 = 本表购电量 × 该时段电价，保留 4 位小数。
# 计划表 / 调整表的目标值不同（不再是同一个「当日总费用」）。
_TGT_PLAN, _TGT_ADJ = 13077598.4544, 12510488.1196
tot = sum(r[146] for r in plan); totA = sum(r[146] for r in adj)
d = abs(tot - _TGT_PLAN) / _TGT_PLAN
dA = abs(totA - _TGT_ADJ) / _TGT_ADJ
out.append("## G2 数值\n")
out.append(f"- 计划表第147列全年合计 = **{tot:.4f}**（方案 A 目标 {_TGT_PLAN:,.4f}；相对差 {d:.2e}）\n"
           f"- 调整表第147列全年合计 = {totA:.4f}（方案 A 目标 {_TGT_ADJ:,.4f}；相对差 {dA:.2e}）\n"
